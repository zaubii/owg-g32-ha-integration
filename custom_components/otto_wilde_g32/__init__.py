...
from .const import CONF_DEVICE_TRACKER, DOMAIN, PLATFORMS
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        await api_client.async_get_grill_details()
        ...
        if entity_id:
            ...
            def _state_change_handler(event, sn=serial_number):
                ...
                if new_state.state == "home":
                    _LOGGER.info(
                        "Tracked device for grill %s is home. Triggering connection check.",
                        sn,
                    )
            ...
            if initial_state and initial_state.state == "home":
                _LOGGER.info(
                    "Initial state for %s is 'home'. Triggering connection check.",
                    entity_id,
                )
