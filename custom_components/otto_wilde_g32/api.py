...
from .const import (
    ...
    PACKET_IDLE_TIMEOUT_SECONDS,
    RAPID_RETRY_ATTEMPTS,
    RAPID_RETRY_DELAY_SECONDS,
    TCP_HOST,
    TCP_PORT,
)
...
class AuthenticationError(Exception):
    """Raised when Otto Wilde credentials are rejected."""


class CannotConnectError(Exception):
    """Raised when the Otto Wilde cloud cannot be reached."""
...
class OttoWildeG32ApiClient:
    ...
    def __init__(
        self, email: str, password: str, session: ClientSession, hass: HomeAssistant
    ):
    ...
    def _is_device_tracker_home(self, serial_number: str) -> bool:
        ...
        if state and state.state == "home":
            _LOGGER.debug(
                "Device tracker %s for grill %s is 'home'.", entity_id, serial_number
            )
            return True

        _LOGGER.debug(
            "Device tracker %s for grill %s is NOT 'home' (state: %s).",
            entity_id,
            serial_number,
    ...
    async def connect_if_needed(self, serial_number: str):
        ...
        if self.is_grill_enabled(serial_number):
            _LOGGER.debug(
                "Connection for grill %s is already enabled/running.", serial_number
            )
            return
    ...
    def sync_counter(
        self, counter_id: str, restored_value: int, serial_number: str | None = None
    ):
        """Syncs the counter with a restored value from Home Assistant."""
        _LOGGER.debug(
            "Sync request for counter '%s' with restored value: %d",
            counter_id,
            restored_value,
        )
        if serial_number:
            current_session_value = self._counters[counter_id].get(serial_number, 0)
            self._counters[counter_id][serial_number] = (
                restored_value + current_session_value
            )
        else:
            if counter_id in self._synced_global_counters:
                _LOGGER.debug(
                    "Global counter '%s' already synced. Skipping.", counter_id
                )
                return
            ...
            _LOGGER.debug(
                "Global counter '%s' synced for the first time this session.",
                counter_id,
            )

        _LOGGER.debug(
            "Sync complete for '%s'. New value: %s",
            counter_id,
    ...
    async def async_get_grill_details(self) -> bool:
        ...
        Raises:
            AuthenticationError: Credentials or token rejected.
            CannotConnectError: Network / transport failure.
        """
        max_retries = 2
        for attempt in range(max_retries):
            ...
            _LOGGER.info(
                "Fetching grill details (Attempt %d/%d)", attempt + 1, max_retries
            )
            ...
            try:
                ...
                if response.status in (401, 403):
                    ...
                    raise AuthenticationError(
                        "Failed to get grill details after re-login attempt."
                    )
                ...
                if err.status in (401, 403):
                    raise AuthenticationError(
                        f"Grill details request rejected: {err.status}"
                    ) from err
                raise CannotConnectError(
                    f"Grill details request failed: {err}"
                ) from err
            except (ClientError, json.JSONDecodeError, TimeoutError) as err:
                raise CannotConnectError(
                    f"Failed to get/parse grill details: {err}"
                ) from err
        return False
    ...
    def _dispatch_data(self, sn: str, data: dict):
        ...
        for cb in self._update_callbacks.get(sn, []):
            cb(data)

    async def _tcp_listener_loop(self, grill_info: dict):
        serial_number = grill_info["serialNumber"]
        pop_key = grill_info["popKey"]
        nickname = grill_info.get("nickname", grill_info["serialNumber"])
        rapid_retry_count = 0

        while self.is_grill_enabled(serial_number):
            # Keep current behaviour: not-home disables Connection Enabled.
            if not self._is_device_tracker_home(serial_number):
                _LOGGER.info(
                    "Device tracker for grill %s is not home. Disabling connection.",
                    nickname,
                )
            ...
            try:
                    ...
                    json.dumps(
                        {
                            ...
                            "data": {
                                "grillSerialNumber": serial_number,
                                "pop": pop_key,
                            },
                        }
                    )
                ...
                first_packet = await asyncio.wait_for(
                    reader.read(1024), timeout=HEARTBEAT_TIMEOUT_SECONDS
                )

                if not first_packet:
                    _LOGGER.warning(
                        "Connection closed by server before first packet for %s.",
                        nickname,
                    )
                else:
                    _LOGGER.info(
                        "Successfully connected and received first packet from: %s",
                        nickname,
                    )
                    connection_succeeded, rapid_retry_count = True, 0
                    self._counters["tcp_reconnect_counter"][serial_number] = 0
                    self._backoff_start_times.pop(serial_number, None)
                    self._diag_next_connection_attempt.pop(serial_number, None)
                    self._dispatch_diagnostics_update(serial_number)
                    ...
                    while self.is_grill_enabled(serial_number):
                        ...
                        if (
                            start_index != -1
                            and len(buffer) >= start_index + PACKET_SIZE
                        ):
                            packet = buffer[
                                start_index : start_index + PACKET_SIZE
                            ]
                            buffer = buffer[start_index + PACKET_SIZE :]
                            if parsed_data := self._parse_binary_data(packet):
                                self._dispatch_data(serial_number, parsed_data)
                        else:
                            try:
                                new_data = await asyncio.wait_for(
                                    reader.read(1024),
                                    timeout=PACKET_IDLE_TIMEOUT_SECONDS,
                                )
                            except asyncio.TimeoutError:
                                _LOGGER.warning(
                                    "No data from %s for %d seconds. Reconnecting.",
                                    nickname,
                                    PACKET_IDLE_TIMEOUT_SECONDS,
                                )
                            ...
                            if not new_data:
                                _LOGGER.warning(
                                    "TCP connection closed during operation for: %s",
                                    nickname,
                                )
                                connection_succeeded = False
                                break
                ...
                _LOGGER.info("Listener task for %s cancelled.", nickname)
                break
            except (ConnectionError, OSError, asyncio.TimeoutError) as err:
                _LOGGER.warning(
                    "TCP connection failed for %s: %s", nickname, type(err).__name__
                )
            finally:
                if writer:
                    writer.close()
            ...
            if connection_succeeded or not self.is_grill_enabled(serial_number):
                if not self.is_grill_enabled(serial_number):
                    break
                continue
            ...
            if rapid_retry_count < RAPID_RETRY_ATTEMPTS:
                _LOGGER.info(
                    "Rapid retry %d/%d for %s in %d seconds.",
                    rapid_retry_count,
                    RAPID_RETRY_ATTEMPTS,
                    nickname,
                    RAPID_RETRY_DELAY_SECONDS,
            ...
            elapsed = datetime.now(timezone.utc) - self._backoff_start_times[
                serial_number
            ]
            if elapsed > timedelta(minutes=OVERALL_TIMEOUT_MINUTES):
                _LOGGER.error(
                    "Grill %s offline for over %d minutes. Disabling connection.",
                    nickname,
                    OVERALL_TIMEOUT_MINUTES,
                )
            ...
            delay = min(
                MAX_RETRY_DELAY_SECONDS, INITIAL_RETRY_DELAY_SECONDS * (2**attempt)
            )
            self._diag_next_connection_attempt[serial_number] = datetime.now(
                timezone.utc
            ) + timedelta(seconds=delay)
            self._counters["tcp_reconnect_counter"][serial_number] = attempt + 1

            _LOGGER.info(
                "Backoff attempt %d for %s. Retrying in %.2f seconds.",
                attempt + 1,
                nickname,
                delay,
            )
    ...
    async def async_login(self) -> bool:
        ...
        try:
            ...
            if response.status in (401, 403):
                raise AuthenticationError("Invalid Otto Wilde credentials")
            response.raise_for_status()
            response_data = await response.json()
            data_payload = response_data.get("data", {})
            access_token = data_payload.get("accessToken")
            if not access_token:
                raise AuthenticationError("Login response missing access token")
            self._tokens = {"access_token": access_token}
            self.user_info = data_payload.get("user", {})
            _LOGGER.info(
                "Login successful for user: %s", self.user_info.get("nickname")
            )
            return True
        except AuthenticationError:
            raise
            ...
            if err.status in (401, 403):
                raise AuthenticationError(
                    f"Login rejected: {err.status}"
                ) from err
            raise CannotConnectError(f"API login failed: {err}") from err
        except (ClientError, json.JSONDecodeError, TimeoutError) as err:
            raise CannotConnectError(f"API login failed: {err}") from err
    ...
    def _parse_binary_data(self, data: bytes) -> dict[str, Any] | None:
        ...
        try:
            return {
                "raw_hex_dump": hex_data,
                "zone_1": self._parse_temp_value(hex_data[12:16]),
                "zone_2": self._parse_temp_value(hex_data[16:20]),
                "zone_3": self._parse_temp_value(hex_data[20:24]),
                "zone_4": self._parse_temp_value(hex_data[24:28]),
                "probe_1": self._parse_temp_value(hex_data[28:32]),
                "probe_2": self._parse_temp_value(hex_data[32:36]),
                "probe_3": self._parse_temp_value(hex_data[36:40]),
                "probe_4": self._parse_temp_value(hex_data[40:44]),
                "gas_weight": int(hex_data[44:48], 16),
                "lid_open": hex_data[48:50] == "01",
                "light_on": hex_data[50:52] == "01",
                "gas_level": int(hex_data[62:64], 16),
                "gas_low": int(hex_data[44:48], 16) < 2200,
            }
    ...
    def _parse_temp_value(self, h: str) -> float | None:
        if h == "9600":
            return None
        try:
            return (int(h[:2], 16) * 10) + (int(h[2:], 16) / 10.0)
    ...
    def register_update_callback(self, sn: str, cb: Callable) -> Callable:
        ...
        def unregister():
            self._update_callbacks[sn].remove(cb)
    ...
    def register_state_callback(self, sn: str, cb: Callable) -> Callable:
        ...
        def unregister():
            self._state_update_callbacks[sn].remove(cb)

        return unregister

    def _dispatch_state_update(self, sn: str):
        for cb in self._state_update_callbacks.get(sn, []):
            cb()
    ...
    def register_diagnostics_callback(self, sn: str, cb: Callable) -> Callable:
        ...
        def unregister():
            self._diagnostics_callbacks[sn].remove(cb)

        return unregister

    def _dispatch_diagnostics_update(self, sn: str):
        data = self.get_diagnostics_data(sn)
        for cb in self._diagnostics_callbacks.get(sn, []):
            cb(data)
    ...
    def _dispatch_global_diagnostics_update(self):
        if not self.grills:
            return
        for grill in self.grills:
            self._dispatch_diagnostics_update(grill["serialNumber"])

    def is_grill_enabled(self, sn: str) -> bool:
        return self._enabled_grills.get(sn, False)
    ...
    async def _start_listener_for_grill(self, sn: str):
        if sn in self._tcp_connections and not self._tcp_connections[sn]["task"].done():
            return
    ...
    async def async_stop_listeners(self):
        ...
        for serial in list(self._tcp_connections.keys()):
            await self._stop_listener_for_grill(serial)
    ...
    def get_diagnostics_data(self, serial_number: str) -> dict[str, Any]:
        return {
            ...
            "tcp_connection_attempts": self._counters["tcp_connection_attempts"].get(
                serial_number, 0
            ),
            "tcp_reconnect_counter": self._counters["tcp_reconnect_counter"].get(
                serial_number, 0
            ),
            "next_connection_attempt": self._diag_next_connection_attempt.get(
                serial_number
            ),
        }
